...
begin
  ...
  if v_gate_ok is false then
    ...
    returning id, ops_mission_proposals.status into v_proposal_id, v_proposal_status;
  ...
  if v_auto is true then
    update ops_mission_proposals
      ...
      returning ops_mission_proposals.status into v_proposal_status;

    v_mission_id := ops_create_mission_from_proposal(v_proposal_id);
  else
    v_proposal_status := v_proposal_status;
    v_mission_id := null;
  end if;
