...
begin
  ...
  if v_gate_ok is false then
    ...
    returning id, status into proposal_id, status;
  ...
  if v_auto is true then
    ...
    v_mission_id := ops_create_mission_from_proposal(proposal_id);
    mission_id := v_mission_id;
  else
    select status into status from ops_mission_proposals where id = proposal_id;
    mission_id := null;
  end if;
...
-- Recover stale steps and finalize missions
create or replace function ops_recover_stale_steps(p_threshold_minutes integer)
returns table (recovered_steps integer) language plpgsql as $$
declare
  v_count integer;
  v_mission_id uuid;
begin
  with stale as (
    ...
    where status = 'running'
      and reserved_at < now() - make_interval(mins => p_threshold_minutes)
    for update
  ), updated as (
    update ops_mission_steps s
      set status = 'failed',
          last_error = 'Stale: no progress within threshold'
    from stale
    where s.id = stale.id
    returning s.mission_id
  )
  select count(*) into v_count from updated;

  for v_mission_id in (select distinct mission_id from updated) loop
    perform ops_maybe_finalize_mission(v_mission_id);
  end loop;

  recovered_steps := v_count;
  return next;
end;
$$;
